...
bool j1Map::CleanUp()
{
	...
	// Remove all layers
	p2List_item<Map_Layer*>* item_layer;
	item_layer = data.map_layer.start;//first node
	while (item_layer != NULL)
	{
		RELEASE(item_layer->data);//és una macro que basicament fa un delete
		item_layer = item_layer->next;//Salta d'un node al següent
	}
	data.map_layer.clear();//Primer s'eliminen els apuntadors i finalment es destrueix la llista
...
bool j1Map::LoadLayer(pugi::xml_node& node, Map_Layer* layer)
{
	...
	layer->data = new uint[layer->height*layer->width];//crees un array de tantes posicions com caselles hi ha(width*heihgt)
